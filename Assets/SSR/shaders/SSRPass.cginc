...
void Hierarchical_ZTrace_SingleSPP(PixelInput i, out half4 RayHit_PDF : SV_Target0, out half4 Mask : SV_Target1)
{
	...
	float3 ScreenPos = GetScreenSpacePos(UV, SceneDepth); // UV是屏幕空间0-1，ScreenPos这个输出是 [-1,1],Z就是SceneDepth
	...
	//float3 ViewDir = GetViewDir(WorldPos, ViewPos); // 拿世界空间坐标－相机空间坐标 是个什么意思？ 这行没有用处，而且是错误的计算式
	...
	half4 H = 0.0;
	if (Roughness > 0.1) {
		H = TangentToWorld(ImportanceSampleGGX(Hash, Roughness), half4(ViewNormal, 1.0)); //这个H 到底是什么空间的？ 只有他是View空间，才能正确解释后面的代码
	} else {
		H = half4(ViewNormal, 1.0);
	}
	float3 ReflectionDir = reflect(normalize(ViewPos), H.xyz); // 前面这段计算都是在视空间之中

	// 从ray 开始，感觉计算都转到屏幕空间了
	float3 rayStart = float3(UV, ScreenPos.z); //都是屏幕空间的值，输入， 这里为啥不更直接一点把UV*2-1搞到 [-1,1]呢，更SS了
	float4 rayProj = mul ( _SSR_ProjectionMatrix, float4(ViewPos + ReflectionDir, 1.0) ); //反射光线投影后的clip pos 
	float3 rayDir = normalize( (rayProj.xyz / rayProj.w) - ScreenPos); // 屏幕空间的 ray方向
	rayDir.xy *= 0.5;
